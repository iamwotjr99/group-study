...
public interface StudyGroupHostService {
     ...
     * @param cmd - StudyGroupApproveCommand(studyGroupId, hostId, applicantId)
     */
    void approveApplication(ApproveStudyGroupCommand cmd);

    /**
     * 방장이 스터디그룹 참여 신청자를 거절
     * Participant의 상태가 "대기중"에서 "거절됨"으로 변경
     * @param cmd - StudyGroupRejectCommand(studyGroupId, hostId, applicantId)
     */
    void rejectApplication(RejectStudyGroupCommand cmd);

    /**
     * 방장이 스터디그룹 참여자를 강퇴
     * Participant의 상태가 "승인됨"에서 "강퇴됨"으로 변경
     * @param cmd - StudyGroupKickCommand(studyGroupId, hostId, participantId)
     */
    void kickParticipation(KickStudyGroupCommand cmd);
}
